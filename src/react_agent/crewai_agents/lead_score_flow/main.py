...
from react_agent.crewai_agents.lead_score_flow.constants import JOB_DESCRIPTION
from react_agent.crewai_agents.lead_score_flow.crews.lead_response_crew.lead_response_crew import LeadResponseCrew
from react_agent.crewai_agents.lead_score_flow.crews.lead_score_crew.lead_score_crew import LeadScoreCrew
from react_agent.crewai_agents.lead_score_flow.types import Candidate, CandidateScore, ScoredCandidate
from react_agent.crewai_agents.lead_score_flow.utils.candidateUtils import combine_candidates_with_scores
import uuid
...
class LeadScoreFlow(Flow[LeadScoreState]):
    ...
    _methods = ['load_leads', 'score_leads',
                'human_in_the_loop', 'write_and_save_emails']

    def __init__(self, inputs):
        self.langgraph_user_input = inputs
        super().__init__()
    ...
    async def score_leads(self):
        ...
        print("\nlanggraph user input\n", self.langgraph_user_input)
            ...
            result = await (
                ...
                .kickoff_async(
                    inputs={
                        ...
                        "job_description": self.langgraph_user_input["job_description"],
    ...
    async def get_user_input(self):
        """
        An async method to get user input, ensuring it works within async tasks.
        """
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, input, "Enter the number of your choice: ")
...
async def kickoff(langgraph_user_input):
    """
    Run the flow.
    """
    ...
    print("\nkickoff\n")
    lead_score_flow = LeadScoreFlow(langgraph_user_input)
    result = await lead_score_flow.kickoff_async()
    return 'lead crew activated'
